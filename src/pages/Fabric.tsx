...
const Fabric: React.FC = () => {
    ...
    useEffect(() => {
        const fetchFabrics = async (): Promise<void> => {
            try {
                setLoading(true);
    ...
    const handleFabricSelect = (fabric: FabricType): void => {
        setSelectedFabric(fabric);
        setFabricConfig({
            ...fabricConfig,
            type: fabric.id,
            color: typeof fabric.colors[0] === 'string' ? fabric.colors[0] : fabric.colors[0].name,
    ...
    const handleConfigChange = (field: keyof FabricConfig, value: string | number | null): void => {
        const newConfig = { ...fabricConfig, [field]: value };
        setFabricConfig(newConfig);
        updateOrderSummary(newConfig);
    };

    const updateOrderSummary = async (config: FabricConfig): Promise<void> => {
                                ...
                                >
                                    <img 
                                        ...
                                        onError={(e: React.SyntheticEvent<HTMLImageElement>) => {
                                            ...
                                            e.currentTarget.src = `/api/placeholder/300/200?text=${fabric.name}`;
                                        }}
                                    />
