// src/server/services/stabilityService.ts
...
const stabilityService = {
  generateImage: async (prompt: string, options: StabilityOptions = {}): Promise<StabilityResponse> => {
    ...
    try {
      ...
      console.error('Primary approach failed:', (error as Error).message);
      ...
      try {
        ...
        console.error('v1 fallback failed:', (v1Error as Error).message);
        ...
        try {
          ...
          throw new Error(`Image generation failed with all approaches: ${(error as Error).message}`);
        }
      }
    }
  }
...
// Primary approach: v2beta with FormData (confirmed working)
async function v2BetaFormDataApproach(apiKey: string, prompt: string, options: StabilityOptions): Promise<StabilityResponse> {
...
async function v1Approach(apiKey: string, prompt: string, options: StabilityOptions): Promise<StabilityResponse> {
...
async function legacyApproach(apiKey: string, prompt: string, options: StabilityOptions): Promise<StabilityResponse> {
  ...
  if (!response.data || !response.data.artifacts || !response.data.artifacts.length) {
    throw new Error('Invalid response format from leg